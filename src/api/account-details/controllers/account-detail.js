...
import AccountDetails from "../models/account-detail.js";
import Address from "../models/account-detail.js";
...
export async function userAccountDetails(req, res) {
  try {
    const token = verify(req);
    ...
    return res.status(200).send({ data: account_details });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
